...
- Một số điều có thể cải thiện thêm cho project:
+ Thêm Dependency Inversion Principle:
Các layer giao tiếp thông qua Interface thay vì trực tiếp
+ Format row id cho mỗi account có tiền tố theo Role
+ Viết unit test
+ Đối với case nhiều table có cùng schema:
Viết hàm để Read sẽ chỉ cần query trên các table dựa các tham số đầu vào mà không cần query trên tất cả các table.
Insert sẽ chỉ cần tạo trên table mới nhất nếu database có chứa nhiều table có cùng schema. Update và Delete có cơ chế search tương tự Read
